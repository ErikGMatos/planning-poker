...
export const VotingRoom: React.FC = () => {
  ...
  const options: (number | string)[] = customVote ? [...baseOptions, customVote, "custom"] : [...baseOptions, "custom"];
        ...
        <div className="voting-room-header">
          <div>
            ...
            <p className="voting-room-subtitle">
              Olá, <span className="user-name">{me.name}</span>! Escolha sua estimativa.
            </p>
          </div>
          ...
          <div className="participants-section">
            <div className="card participants-card">
              <div className="participants-header">
                <h3 className="participants-title">👥 Participantes ({allUsers.length}/{MAX_PARTICIPANTS})</h3>
                <p className="participants-count">{votedCount} de {allUsers.length} votaram</p>
                {allUsers.length >= MAX_PARTICIPANTS - 2 && (
                  <p style={{ fontSize: '0.75rem', color: allUsers.length >= MAX_PARTICIPANTS - 1 ? '#f87171' : '#fbbf24', marginTop: '0.25rem' }}>
                    {allUsers.length === MAX_PARTICIPANTS ? '🚫 Sala lotada' : `⚠️ ${MAX_PARTICIPANTS - allUsers.length} vaga(s) restante(s)`}
                  </p>
                )}
              </div>
                  ...
                  <div key={user.id} className="participant-item">
                    ...
                    <div className="participant-status">
                      {reveal ? (
                        ...
                        <span className={`badge ${user.vote !== null ? 'badge-green' : 'badge-yellow'}`}>
                          {user.vote !== null ? "✓" : "⏳"}
                        </span>
                      )}
                    </div>
